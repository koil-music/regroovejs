    ...
    await gotGenerator.load(filepath)
      ...
      for (let i = 0; i < length; i++) {
        for (let j = 0; j < length; j++) {
          assert.ok(
            arraysEqual(
              Array.from(testDataMatrix[i][j]),
              Array.from(gotGenerator.onsets.data[i][j])
            )
          );
          assert.ok(
            arraysEqual(
              Array.from(testDataMatrix[i][j]),
              Array.from(gotGenerator.velocities.data[i][j])
            )
          );
          assert.ok(
            arraysEqual(
              Array.from(testDataMatrix[i][j]),
              Array.from(gotGenerator.offsets.data[i][j])
            )
          );
        }
      }
