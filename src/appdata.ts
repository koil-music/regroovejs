...
interface IAppData {
  ...
  userDir: string
  _fileExt: string
  _loadIndexFromJSON: (indexData: Record<string, Record<string, string>>) => Record<string, FileMeta>
  loadPattern: (name: string) => Promise<[Pattern, Pattern, Pattern]>
  savePattern: (onsets: Pattern, velocities: Pattern, offsets: Pattern, name: string) => void
}
...
class AppData implements IAppData {
  ...
  _loadFactoryIndexData (index: Record<string, FileMeta>): Record<string, FileMeta> {
    glob(this.factoryDir + this._fileExt, function (err, files: string[]) {
